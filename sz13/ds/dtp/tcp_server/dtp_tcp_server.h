#ifndef _DTP_TCP_SERVER_H
#define _DTP_TCP_SERVER_H
...
class DTPTCPServer: public DTPServer
{
public:
    static DTPTCPServer* New(UINT16 usInstID, UINT8 *pucAddr, UINT16 usPort,
    ...
    virtual BOOL  GetClientAddr(UINT16 usClientID, UINT8 *pucClientAddr, UINT16 *pusClientPort);
    ...
    VOID    CloseConnnect(DTP_CONN_INFO_T *pstConn);
    BOOL    SetConnectClose(SOCKET stSocket);
    VOID    PrintConnect();

    BOOL    TcpSend(SOCKET stSocket, DTP_MSG_HDR_T *pstMsg, UINT32 ulMsgLen, BOOL bNeedLock=TRUE);
...
private:
    ...
    DTPTCPServer(DTPServerMsgHandler *pMsgHandle, GMemAlloc *pMemAlloc);

    BOOL Init(UINT16 usInstID, UINT8 *pucAddr, UINT16 usPort);

    DTP_MSG_T* AllocMsgNode(UINT32 ulMsgLen);
    BOOL SendEx(SOCKET stSocket, UINT16 usServerModuleID, UINT16 usClientID, UINT16 usClientModuleID,
               UINT16 usMsgType, UINT32 ulSeqID, VOID *pvMsg, UINT32 ulMsgLen, UINT32 ulTimeout=0);

    BOOL HandleLinkAuthReqMsg(DTP_CONN_INFO_T *pstConn, DTP_TCP_LINK_AUTH_REQ_T *pstReq);
    BOOL HandleLinkReleaseReqMsg(DTP_CONN_INFO_T *pstConn, DTP_LINK_RELEASE_REQ_T *pstReq);

    BOOL HandleClientMsg(SOCKET stClientSocket, DTP_MSG_T *pstMsg);
    BOOL HandleLinkCheckReqMsg(DTP_CONN_INFO_T *pstConn, DTP_MSG_T *pstReq);
    BOOL SendBySocket(SOCKET stSocket, DTP_MSG_HDR_T *pstMsgHdr);

    VOID ClientLinkCheck(DTP_CONN_INFO_T *pstConn, BOOL *pucNeedClose);

    INT32 ListenMsg(UINT32 ulTimeout);
    INT32 RecvMsg(DTP_CONN_INFO_T *pstConn, INT32 *piErrCode);
    UINT32 CheckRecvMsg(SOCKET stSocket, UINT8 *pucMsg, UINT32 ulTotalMsgLen, UINT32 *pulMsgSize);
    ...
    VOID _InitConnectInfo(DTP_CONN_INFO_T *pstConn);
    VOID _InitConnectInfo(SOCKET stSocket, BOOL bNeedMutex);
    BOOL _SetConnectInfo(SOCKET stSocket, SOCKADDR_IN *pstAddr);

    BOOL SendNow(SOCKET stSocket, UINT16 usServerModuleID, UINT16 usClientID, UINT16 usClientModuleID,
