...
BOOL DTPClientImp::HandleLinkCheckReq(DTP_LINK_CHECK_REQ_T *pstReq)
{
    ...
    if (!TcpSend(m_pstClient->stSocket, &stRsp, sizeof(stRsp)))
    {
...
BOOL DTPClientImp::AuthLink()
{
    ...
    DTPLog(LOG_INFO, "start link auth");
    ...
    if (!TcpSend(m_pstClient->stSocket, &stReq, sizeof(stReq), FALSE))
    {
...
again:
    ...
    iRecvMsgLen = dtp_tcp_recv(m_pstClient->stSocket, &stRsp, sizeof(stRsp), iWaitTime);
    ...
    if (stRsp.stMsgHdr.ulMsgMagic != DTP_MSG_MAGIC)
    {
        DTPLog(LOG_FATAL, "client_auth_link: link auth failed, invalid msg magic %08X", stRsp.stMsgHdr.ulMsgMagic);
        return FALSE;
    }

    if (stRsp.stMsgHdr.usMsgType != DTP_EV_LINK_AUTH_RSP )
    {
        DTPLog(LOG_FATAL, "client_auth_link: link auth failed, invalid msg type %u", stRsp.stMsgHdr.usMsgType);
        return FALSE;
    }
    ...
    memcpy(m_pstClient->aucClientAddr, stRsp.aucClientAddr, sizeof(m_pstClient->aucClientAddr));
    m_pstClient->usClientPort = stRsp.usClientPort;
    m_pstClient->usClientID = stRsp.usClientID;
    DTPLog(LOG_FATAL, "client_auth_link: tcp link auth successful, client id is %u", m_pstClient->usClientID);
