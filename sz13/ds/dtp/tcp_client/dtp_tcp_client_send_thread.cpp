...
UINT32 DTPClientImp::_GetSeqID()
{
    ...
    if (m_pstClient->ulSeqID == 0)
    {
        m_pstClient->ulSeqID = 1;
    }
...
BOOL DTPClientImp::TcpSend(SOCKET stSocket, VOID *pvMsg, UINT32 ulMsgLen, BOOL bNeedLock)
{
    ...
    BOOL bRet = dtp_tcp_send(stSocket, pvMsg, ulMsgLen);
                          ...
                          VOID *pvMsg, UINT32 ulMsgLen, UINT32 ulTimeout)
{
    ...
    BOOL bRet = TcpSend(m_pstClient->stSocket, pstMsg, ulTotalMsgLen);
...
GOS_THREAD_RET DTPTCPClientSendThreadEntry(VOID *pvData)
{
    UINT32                  ulMsgLen;
...
/*        pstMsg = (DTP_MSG_T *)pstMsgNode->pucMsg;
        ...
        if (pClient->TcpSend(pstClient->stSocket, pstMsgNode->pucMsg, ulMsgLen, FALSE))
        {
