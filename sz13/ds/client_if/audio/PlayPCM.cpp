...
#if 1
typedef HANDLE (*FUNC_PCMPlayInit)(UINT32 ulChanNum, UINT32 ulSampleRate, UINT32 ulBitsPerSample);
typedef VOID (*FUNC_PCMPlayStart)(HANDLE hPcmPlay);
typedef VOID (*FUNC_PCMPlayPushData)(HANDLE hPcmPlay, VOID *pvData, UINT32 ulLen);
typedef VOID (*FUNC_PCMPlayStop)(HANDLE hPcmPlay);

FUNC_PCMPlayInit        m_funcPCMPlayInit;
FUNC_PCMPlayStart       m_funcPCMPlayStart;
FUNC_PCMPlayPushData    m_funcPCMPlayPushData;
FUNC_PCMPlayStop        m_funcPCMPlayStop;
...
BOOL _PCMPlayInit()
{
    ...
    HMODULE hDll = LoadLibrary("PcmPlay.dll");
    ...
    m_funcPCMPlayInit =(FUNC_PCMPlayInit)GetProcAddress(hDll, "PCMPlayInit");
    m_funcPCMPlayStart =(FUNC_PCMPlayStart)GetProcAddress(hDll, "PCMPlayStart");
    m_funcPCMPlayPushData =(FUNC_PCMPlayPushData)GetProcAddress(hDll, "PCMPlayPushData");
    m_funcPCMPlayStop =(FUNC_PCMPlayStop)GetProcAddress(hDll, "PCMPlayStop");

    if (!m_funcPCMPlayInit ||
        !m_funcPCMPlayStart ||
        !m_funcPCMPlayPushData ||
        !m_funcPCMPlayStop)
    {
        m_funcPCMPlayInit = NULL;
        return FALSE;
    }
...
HANDLE PCMPlayInit(UINT32 ulChanNum, UINT32 ulSampleRate, UINT32 ulBitsPerSample)
{
    ...
    return m_funcPCMPlayInit(ulChanNum, ulSampleRate, ulBitsPerSample);
}

VOID PCMPlayStart(HANDLE hPcmPlay)
{
    if (!m_funcPCMPlayStart)
    {
        return;
    }

    m_funcPCMPlayStart(hPcmPlay);
}

VOID PCMPlayPushData(HANDLE hPcmPlay, VOID *pvData, UINT32 ulLen)
{
    if (!m_funcPCMPlayPushData)
    {
        return;
    }

    m_funcPCMPlayPushData(hPcmPlay, pvData, ulLen);
}

VOID PCMPlayStop(HANDLE hPcmPlay)
{
    if (!m_funcPCMPlayStop)
    {
        return;
    }

    m_funcPCMPlayStop(hPcmPlay);
}
...
//#pragma comment(lib, "../lib/dxguid_bcb.lib")
#pragma comment(lib, "../lib/dxguid_vc.lib")
...
#define BUFFERNOTIFYSIZE    384000
...
BOOL PlatPCM()
{
    int     i;
    FILE    *fp;
    char    acFile[] = "d://test1.wav";

    if ((fp=fopen(acFile,"rb"))==NULL)
    {
        printf("cannot open this file\n");
        return FALSE;
    }

    fseek(fp, SEEK_SET, 78);
