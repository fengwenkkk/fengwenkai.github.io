    ...
    : TSubForm(Owner)
{
    ...
    System::Classes::TStringList  *pstrTestList = new TStringList;
    //strTestList->Clear();

    //tlMain->OptionsData->Editing = true;
    ...
    //TcxComboBoxProperties *tt = new TcxComboBoxProperties(ColumnTest5->Properties);
    //TcxComboBoxProperties(ColumnTest5->Properties)->Items->AddStrings(pstrTestList);
    TcxComboBoxProperties *tt = (TcxComboBoxProperties *)ColumnTest5->Properties;

//    int x = tt->Items->Count;
    ...
    tt->Items->AddStrings(pstrTestList);

    //tt->Items->AddPair("全车","255");

    //tt->Items->Add("123");
    //tt->Items->Add("321");

    //tt->Items->AddPair("全车","255");
    //tt->Items->AddPair("1车厢","1");
}
...
void __fastcall TFormPIS::OnPISMsg(UINT8 *pucMsg, UINT32 ulMsgLen)
{
    ...
    if (pstMsg->usMsgType == EV_PIS_SELFCHECK_STATUS)
    {
		//OnGetSelfCheckReq((PIS_SELFCHECK_STATUS_REQ_T*)pvMsgData);
    }
    else if (pstMsg->usMsgType == EV_PIS_HEART_BEAT)
    {
		OnSAGetIPHSysInfoReq();
    }
...
{
#if 1
    if (!FileExists(edtWavFile->Text.c_str()))
    {
        return;
    }
