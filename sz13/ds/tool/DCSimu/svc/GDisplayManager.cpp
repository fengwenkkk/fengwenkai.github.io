...
BOOL    g_bMDCConnected = FALSE;

GDisplayManager::GDisplayManager(void)
{
...
void GDisplayManager::notifyUserStatus(ResourceStatusIndicator * resourceStatus)
{
...
void GDisplayManager::notifyP2pvideocallStatus(P2pvideocallStatusIndicator * callStatus)
{
...
//通知用户普通短消息的服务接收实现
void GDisplayManager::notifySMS(SdsMessageIndicator * sdsmsg)
{
...
//通知用户状态短消息的服务接收实现
void GDisplayManager::notifyUserSdsStatus(SdsMessageIndicator * sdsmsg)
{
...
void GDisplayManager::notifyResourceStatus(ResourceStatusIndicator * resourceStatus)
{
...
void GDisplayManager::notifyP2pcallStatus(P2pcallStatusIndicator * callStatus)
{
...
void GDisplayManager::notifyGroupStatus(GrpDspInfo * grpInfo)
{
  if(grpInfo == NULL)
  {
...
void GDisplayManager::notifyModuleStatus(DCModuleStatusIndicator * moduleStatus)
{
    ...
    module_status_t     stModuleStatus = moduleStatus->getModuleStatus();

    if (stModuleStatus == RESOURCE_STATUS_NORMAL)
    {
        g_bMDCConnected = TRUE;
    }
    else if (stModuleStatus == MODULE_DISCONNECT)
    {
        g_bMDCConnected = FALSE;
    }
}


void GDisplayManager::notifyProvisionChange(ProvChangeIndicator* provch)
{
