...
TaskSys::TaskSys(UINT16 usDispatchID):GBaseTask(MODULE_SYS, (CHAR*)"SYS", usDispatchID)
{
    ...
    memcpy(m_aucNTPAddr, g_aucNTPAddr, sizeof(g_aucNTPAddr));
...
VOID TaskSys::NTPSync()
{
    ...
    INT32   iTime = gos_get_ntp_time(m_aucNTPAddr, g_usNTPPort);

    if (iTime < 0)
    {
        TaskLog(LOG_INFO, "connect to NTP server " IP_FMT " failed!", IP_ARG(m_aucNTPAddr));
        g_bNTPValid = FALSE;
        return;
    }
