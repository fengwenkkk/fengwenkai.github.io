...
void __fastcall TFormQuery::InitSkin()
{
    ...
    for (UINT32 i=0; i<ARRAY_SIZE(btns); i++)
    {
        btns[i]->LookAndFeel->SkinName = UICfg::GetRightToolbarButtonSkinName();

        btns[i]->Colors->DefaultText = GetDefaultTextColor();
        btns[i]->Colors->HotText = btns[i]->Colors->DefaultText;
    }
}
//---------------------------------------------------------------------------
