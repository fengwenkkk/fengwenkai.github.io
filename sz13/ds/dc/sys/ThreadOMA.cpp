...
BOOL ThreadOma::SendDiscoveryTrap()
{
    ...
    INT32                   iRet = -1;
    CHAR                    acModelName[32] = {OMC_MODEL_NAME_DC};
    UINT32                  ulDevType = OMC_DEV_TYPE_DC;
...
{
    szConfigFile = gos_get_simple_file_name(szConfigFile);
                              ...
                              unsigned int ulMaxOIDNum, unsigned int *pulOIDNum)
{
    ...
    *pulGetType = *pucRspData++;
    ...
    if (pucRspData[0] != ASN_INTEGER ||
        pucRspData[1] == 0)
    {
        OmaLog(LOG_ERROR, "snmp_parse_get_req: pucRspData[0]:%u,pucRspData[1]:%u.",
            pucRspData[0],
            pucRspData[1]);
        OmaLog(LOG_ERROR, "snmp_parse_get_req: check reqid failed");
        return -1;
    }
...
BOOL ThreadOma::CheckAlarm()
{
    ...
    if (!m_bSendRebootAlarm &&
        gos_file_exist(acRebootIndFile))
    {
