...
extern UINT32  g_ulPALinkcheckMaxNum;
...
BOOL SendPAMsg(UINT16 usMsgType, VOID *pvMsg, UINT16 usMsgLen)
{
    ...
    {
        GosLog(LOG_ERROR, "send PA msg failed for invalid socket");
        return FALSE;
    }
...
VOID SendPAHeartBeatMsg()
{
    ...
    static UINT32   ulLastPAMsgTime = 0;

    if (ulLastPAMsgTime == 0 ||
        ulCurrTime >= (ulLastPAMsgTime+g_stSACfg.ulPISHeartBeatTime*1000))
    {
        SendPAMsg(EV_PA_GET_SYS_STATUS);
        ulLastPAMsgTime = ulCurrTime;

        GosLog(LOG_DETAIL, "send PA heartbeat end");
    }
}

BOOL GetSelfCheckStatus(INT32 *piAppSelfCheckStatus)
{
    INT32 iValue = -1;

    if (ram_get(APP_STATUS_SelfCheckStatus, &iValue) == -1)
    {
        GosLog(LOG_ERROR, "get self check status failed");
        return FALSE;
    }

    if (iValue != APP_SELF_CHECK_STATUS_SUCC   &&
        iValue != APP_SELF_CHECK_STATUS_FAILED &&
        iValue != APP_SELF_CHECK_STATUS_UNKNOWN)
    {
        GosLog(LOG_ERROR, "invalid self check status %d", iValue);
        return FALSE;
    }

    *piAppSelfCheckStatus = iValue;
...
VOID SendPASelfCheckMsg()
{
    ...
    if (ulTime >= ulSendPeriod)
    {
        ...
        GosLog(LOG_DETAIL, "send  app self check status to PA end, app self check status is %u", stMsg.ucCheckStatus);
    }
...
VOID SendPAGetTrainInfoMsg()
{
    ...
    GosLog(LOG_DETAIL, "send PA get train info msg");
    ...
    GosLog(LOG_DETAIL, "send PA get train info msg end");
}
...
static VOID OnPAStartBrdRsp(VOID *pvMsg, UINT32 ulMsgLen)
{
    ...
    NTOHL_IT(pstRsp->ulBrdSeqID);

    GosLog(LOG_INFO, "recv PA_REAL_BRD_RSP msg, brd seq id %u", pstRsp->ulBrdSeqID);

    // 发送消息给DIS
    SendSAStartRealBrdRspMsg(pstRsp);
}

static VOID OnPAEndBrdRsp(VOID *pvMsg, UINT32 ulMsgLen)
{
    PA_STOP_REAL_BRD_RSP_T  *pstRsp = (PA_STOP_REAL_BRD_RSP_T*)pvMsg;

    if (ulMsgLen != sizeof(PA_STOP_REAL_BRD_RSP_T))
    {
        GosLog(LOG_ERROR, "invalid msg len(%u) of PA_STOP_REAL_BRD_RSP", ulMsgLen);
        return;
    }

    NTOHL_IT(pstRsp->ulBrdSeqID);

    GosLog(LOG_INFO, "recv PA_STOP_REAL_BRD_RSP msg, brd seq id %u", pstRsp->ulBrdSeqID);
...
static VOID OnPAGetTrainInfoRsp(VOID *pvMsg, UINT32 ulMsgLen)
{
        ...
        pstRsp->ucDriverMode  != ucDriverMode )
    {
        ...
        ram_set(TRAIN_COVER_STATUS, (UINT32)ucCoverStatus);
        ram_set(TRAIN_DRIVE_MODE, (UINT32)ucDriverMode);
//        CfgCommit();
    }
...
{
    GosLog(LOG_INFO, "recv PIS_SELFCHECK_STATUS msg");
...
static VOID OnPARecBrdRsp(VOID *pvMsg, UINT32 ulMsgLen)
{
    ...
    NTOHL_IT(pstRsp->ulRecID);        // 预录音广播编号
    NTOHL_IT(pstRsp->ulBrdSeqID);  

    GosLog(LOG_INFO, "recv PA_REC_BRD_RSP msg, brd seq id %u, result is %u", pstRsp->ulBrdSeqID, pstRsp->ucRet);
...
static VOID OnPAHeartBeat(VOID *pvMsg, UINT32 ulMsgLen)
{
    ...
    g_ulPAMasterStatus = pstRsp->ucIsMaster;
    g_ulPARunStatus = pstRsp->ucStatus;

    ram_set(TRAIN_PA_RUNNING_STATUS, g_ulPARunStatus);
    ram_set(TRAIN_PA_MASTER_STATUS, g_ulPAMasterStatus);
