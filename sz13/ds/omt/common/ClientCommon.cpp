...
BOOL GetGsonResult(GJson &RspJson)
{
    ...
    SubJson.GetValue("Type", &ulOmtType);//ZWJ  获取OMC返回的数据库中的用户类型

    if (szResult && strcasecmp(szResult, "Succ") == 0)
    {
        //g_stLocalCfg = ulOmtType; //将从OMC返回的用户类型保存到全局变量 ZWJ
        g_stLocalCfg.ulOmtUserType = ulOmtType;
        return TRUE;
    }
...
BOOL SyncSendServer(UINT16 usMsgID, const CHAR *szMsgName, GJsonParam &MsgInfoParam, GJson &RspSubJson, UINT32 ulTimeout)
{
    ...
    ReqJsonParam.Add("UserID", g_stLocalCfg.acUserName);
...
VOID SaveDCOperator(CHAR *szOperator, BOOL bOffDuty)
{
    ...
    AddConfirmRequest(EV_DC_DIS_REQ, MSG_DCSetOperator, Param, ulRetryTime);
}
 */
...
BOOL HttpDownloadFile(AnsiString strUrl, AnsiString strLocalFile)
{
    ...
    CHAR        *szUser = g_stLocalCfg.acUserName;
    CHAR        *szPwd = g_stLocalCfg.acUserPwd;

    return DownHttpFile(szUrl, szLocalFile, szUser, szPwd);
}
