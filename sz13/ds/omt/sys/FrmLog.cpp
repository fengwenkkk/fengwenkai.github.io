...
*/
	VECTOR<OMT_OPER_LOG_INFO_T>     vInfo;

	if (!LoadOperLogInfo(ulStartTime, ulEndTime, acOperatorID, vInfo))
	{
		InfoMsg("查询失败");
		return;
	}
...
//---------------------------------------------------------------------------
void __fastcall TFormLog::InitView(VECTOR<OMT_OPER_LOG_INFO_T> &vInfo)
{
    TcxTreeListNode     *Node;
	OMT_OPER_LOG_INFO_T     *pstInfo;
    BOOL                bDesc = FALSE;
    ...
    for (UINT32 i=0; i<vInfo.size(); i++)
    {
        ...
        Node->Values[3] = pstInfo->acUserID;
