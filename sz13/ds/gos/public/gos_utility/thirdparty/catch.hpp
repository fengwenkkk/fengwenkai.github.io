...
#ifdef __BORLANDC__
    static CopyableStream s_CopyableStream;
    static CopyableStream& m_stream()
    {
        return s_CopyableStream;
    }
#else
    static CopyableStream& m_stream()
    {
...
inline std::vector<std::string> argsToVector(int argc, char const* const* const argv)
{
    ...
    for (std::size_t i = 0; i < static_cast<std::size_t>(argc); ++i) args[i] = argv[i];
...
public:
    ...
    void parseIntoTokens(std::vector<std::string> const& args, std::vector<Token>& tokens)
    {
        ...
        for (std::size_t i = 1; i < args.size() && args[i] != doubleDash; ++i) parseIntoTokens(args[i], tokens);
    }
    ...
    Text(std::string const& _str, TextAttributes const& _attr = TextAttributes()) : attr(_attr)
    {
        ...
        while (it != strEnd)
        {
            ...
            if (itEnd != strEnd)
            {
                ...
                {
                    while (itEnd > it && wrappableInsteadOfChars.find(*(itEnd - 1)) != std::string::npos) --itEnd;
                }
            }
...
std::string toString(std::wstring const& value)
{
    ...
    for (size_t i = 0; i < value.size(); ++i) s += value[i] <= 0xff ? static_cast<char>(value[i]) : '?';
...
#define CATCH_CHECK_NOFAIL(expr)              \
    INTERNAL_CATCH_TEST("CATCH_CHECK_NOFAIL", \
                        Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::SuppressFail, expr)
...
#define REQUIRE_FALSE(expr) \
    INTERNAL_CATCH_TEST("REQUIRE_FALSE", Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, expr)
#endif
