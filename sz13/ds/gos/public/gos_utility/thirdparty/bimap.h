 ...
 *
 * (C) 2002-2006 Joaqu┴n M L┏pez Mu?oz (joaquin@tid.es). All rights reserved.
 *
 ...
 *       "Portions copyright 2002-2006 Joaqu┴n M L┏pez Mu?oz (joaquin@tid.es)"
 *
...
struct equal_types
{
    enum
    {
        value = (sizeof(equal_types_helper(equal_types_ptr<T>::make(), equal_types_ptr<U>::make())) ==
...
protected:
    ...
    {
    public:
        const_from_binding(const bimap_type& bm, const typename bimap_type::from_type& f) : bm(bm), f(f) {}
    ...
    {
    public:
        const_to_binding(const bimap_type& bm, const typename bimap_type::to_type& t) : bm(bm), t(t) {}
...
public:
    ...
    public:
        ...
        public:
            ...
            const typename const_iterator::value_type* operator->() const
            {
                return &operator*();
            }
#endif
...
#if defined(_MSC_VER) && _MSC_VER == 1200 /* MSVC++ 6.0 */
        typedef std::reverse_bidirectional_iterator<const_iterator, const value_type, const_reference,
        ...
        from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> > operator[](
            const from_type_& f)
        {
            return from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> >(owner(), f);
        }

        const_from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> > operator[](
            const from_type_& f) const
        {
            return const_from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> >(
                owner(), f);
        }
    ...
    public:
        ...
        to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> > operator[](
            const to_type_& t)
        {
            return to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> >(owner(), t);
        }

        const_to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> > operator[](
            const to_type_& t) const
        {
            return const_to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> >(owner(),
                                                                                                                t);
        }
    ...
    std::pair<typename from_impl::iterator, typename to_impl::iterator> insert(typename from_impl::iterator fit,
...
#ifdef _MSC_VER /* see note in from::erase */
    to_iterator erase(to_iterator it)
    {
...
public:
    ...
    explicit bimap(const inv_bimap& r) : super(r.to.key_comp(), r.from.key_comp(), allocator_type())
#else
    typedef bimap<to_type_, from_type_, to_compare, from_compare,
                  typename allocator_type::template rebind<direct_pair<const to_type_, const from_type_> >::other>
        inv_bimap;

    explicit bimap(const inv_bimap& r) : super(r.to.key_comp(), r.from.key_comp(), r.get_allocator())
#endif
...

    {
        try
        {
            ...
            for (typename inv_bimap::to::const_iterator it = r.to.begin(); it != r.to.end(); ++it)
            {
                std::pair<typename super::from::iterator, typename super::to::iterator> p =
                    this->insert(fhint, thint, *it);
                fhint = p.first;
                thint = p.second;
            }
#endif
