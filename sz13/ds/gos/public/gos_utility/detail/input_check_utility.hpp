...
{
    input_check obj;
    if (!obj.AddRules(gos::count_not_equal, gos::AlwaysTrueChar, 0)   ///< 不能为空
             .AddRules(gos::all_of, gos::IsDigitOrDot)                ///< 所有都是数字或者 '.'
             .AddRules(gos::count_equal, gos::IsChar<'.'>, 3)         ///< 字符 '.' 只能有三个
             .AddRules(gos::count_great_equal, gos::IsDigitOrDot, 7)  ///< 字符数量大于等于七个
             .AddRules(gos::count_less_equal, gos::IsDigitOrDot, 15)  ///< 字符数量最多15个
             .IsValid(strIP))
    {
        return false;
    }
