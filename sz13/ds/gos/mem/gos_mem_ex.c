...
static BOOL     s_bInitUseFlag = FALSE;
static UINT32   s_aulFreeIndex[256];
...
{
    BOOL                    bUseLock;
    UINT32                  ulSubBlockSize; /* 子内存块大小（1个内存块=8个子内存块） */
    UINT32                  ulBlockSize;    /* 内存块大小 */
    UINT32                  ulSubBlockNum;  /* 子内存块个数 */
...
#define GOS_MEM_POOL_SIZE   (256/8)             /* 典型应用为64 - 8*1024*1024  */
                        ...
                        BOOL                    bUseLock)
{
    ...
    if (FALSE == s_bInitUseFlag)
    {
        ...
        for (i=0; i<256; i++)
        {
            ...
            {
                s_aulFreeIndex[i] = 0xff;
            }
        }
    }

    pstMemPools = malloc(sizeof(GOS_MEM_POOLS_T));
    if (NULL == pstMemPools)
    {
        goto fail;
    }
    ...
    for (i=0; i<ulMemPoolNum; i++)
    {
        ...
        {
            if (pstMemPool->ulBlockSize <= pstMemPools->astMemPool[pstMemPools->ulMemPoolNum-1].ulSubBlockSize)
            {
                goto fail;
            }
        }
        ...
        if (pstMemPools->ulMemPoolNum >= GOS_MEM_POOL_SIZE ||
            pstMemPool->ulBlockSize > 0xfffffff)
        {
            goto fail;
        }
        ...
        pstPool->ulSubBlockSize = pstMemPool->ulBlockSize;
        pstPool->ulBlockSize    = (sizeof(UINT32)+pstPool->ulSubBlockSize)*8;
        pstPool->ulBlockNum     = MOD(pstMemPool->ulBlockNum+7,8);
        pstPool->ulSubBlockNum  = pstPool->ulBlockNum*8;
        ...
        pstPool->pucBlockAddr = malloc(pstPool->ulBlockNodeSize*pstPool->ulBlockNum);
        ...
        pstPool->pucStartAddr = pstPool->pucBlockAddr+STRUCT_OFFSET(GOS_MEM_BLOCK_T, aucAddr);
...
VOID gos_mem_print(HANDLE hMemPool, FILE *fp)
{
    ...
    for (i=0; i<GOS_MEM_POOL_SIZE; i++)
    {
        ...
        fprintf(fp, "\n mem pool: size = %u num = %u used = %u max used = %u",
                pstPool->ulSubBlockSize,
                pstPool->ulSubBlockNum,
...
#endif
{
    ...
    {
        pstPool = &pstMemPools->astMemPool[i];
        if ((pstPool->ulSubBlockSize) < ulMemSize)
        {
            continue;
        }
        ...
        ulSubBlockIndex = s_aulFreeIndex[pstNode->ucUseFlag];
        if (ulSubBlockIndex >= 8)
        {
            continue;
        }
        ...
        pulAddr = (UINT32*)(pstNode->aucAddr + ulSubBlockIndex*(sizeof(UINT32)+pstPool->ulSubBlockSize));
        *pulAddr = (ulSubBlockIndex>>28) | ulSubBlockIndex*(sizeof(UINT32)+pstPool->ulSubBlockSize);

        gos_mempool_unlock(pstPool);

        return (VOID*)(pulAddr+1);
    }
...
dyna_alloc:
    ...
    {
        pstNode = malloc(sizeof(UINT32) + ulMemSize + STRUCT_OFFSET(GOS_MEM_BLOCK_T, aucAddr));
        if (NULL == pstNode)
        {
            return NULL;
        }
        ...
        pulAddr = (UINT32*)(pstNode->aucAddr);
        *pulAddr = 0;
...
#endif
{
    ...
    pstNode = (GOS_MEM_BLOCK_T*)((UINT8*)pMem - STRUCT_OFFSET(GOS_MEM_BLOCK_T, aucAddr));
    ...
    {
        // 异常
        if (pstNode->ulMemSize > (pstPool->ulSubBlockSize))
        {
            return pMem;
        }

        // 不要重新申请
        if (ulMemSize <= (pstPool->ulSubBlockSize))
        {
            pstNode->ulMemSize = ulMemSize;
            return pMem;
        }
...
#endif
{
    ...
    UINT32                  ulOffset = GET_INT(pucAddr-4);
    UINT8                   ucBlockSubIndex = (ulOffset >> 28);

    ulOffset = ulOffset & 0x03ffffff;
    ...
    pstNode = (GOS_MEM_BLOCK_T*)(pucAddr - STRUCT_OFFSET(GOS_MEM_BLOCK_T, aucAddr) - ulOffset);
    if (pstNode->ulMagic != GOS_MEM_MAGIC)
    {
        return;
    }
    ...
    pstPool = pstNode->pPool;
    if (NULL == pstPool)
    {
        return;
    }

    if (pucAddr != pstPool->pucStartAddr + pstNode->ulBlockIndex*pstPool->ulBlockNodeSize + ucBlockSubIndex*pstPool->ulBlockNodeSize/8)
    {
        return;
    }
...
VOID gos_mem_print_usage(HANDLE hMemPool, FILE *f, BOOL bPrintAll)
{
    ...
    for (i=0; i<pstMemPools->ulMemPoolNum; i++)
    {
        ...
        fprintf(f, "\n\n gos_mem_usage:: %u", pstPool->ulSubBlockSize);
