...
static VOID byteReverse(UINT8 *buf, unsigned longs)
{
    UINT32 t;
    do
    {
...
void MD5Update(struct MD5Context *ctx, UINT8 *buf, UINT32 len)
{
    UINT32 t;
...
VOID gos_md5(const CHAR *szText, UINT8 *pucResult)
{
    ...
    MD5Init(&ctx);
    MD5Update(&ctx, (UINT8 *)szText, strlen(szText));

    MD5Final(pucResult, &ctx);
}
