...
static __inline ui32 hash_func1(ui32 i)
{
#if QLZ_COMPRESSION_LEVEL == 2
...
#define TEST_PERF(i, j) \
{\
    gos_time_get_tick(&g_i64Tick[i][0]); \
    g_i64Tick[i][1] += g_i64Tick[i][0] - g_i64Tick[j][0]; \
    g_i64Tick[i][2] ++;\
}
#endif
...
int         g_iNum = 0;
__int64     g_iTick3 = 0;
#define START_PERF      gos_time_get_tick(&g_iTick1)
#define END_PERF        gos_time_get_tick(&g_iTick2); g_iTick3 += g_iTick2 - g_iTick1; g_iNum++

int iNum[6] = {0,0,0,0,0,0};
#endif
...
//    memset(g_i64Tick, 0, sizeof(g_i64Tick));
...
#if 0
                //sTmp1 = GET_SHORT(o + matchlen);
                //sTmp2 = GET_SHORT(src);
                do
                {
                    if (GET_SHORT(o + matchlen) == GET_SHORT(src))
                    {
                        //iNum[1] ++;
    ...
    */
    ...
    printf("\n time = %u", gos_time_get_interval_ms(0, g_i64Tick[10][1]));

    iTime = gos_time_get_interval_ms(0, g_iTick3);
    printf("\n ----- %u %u %.2f", iTime, g_iNum, iTime/((double)g_iNum/1000000));

    printf("\n ----- %u %u %u %u %u %u", iNum[0], iNum[1], iNum[2], iNum[3], iNum[4], iNum[5]);
