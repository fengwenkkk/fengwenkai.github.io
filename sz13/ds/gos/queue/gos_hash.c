...
#ifdef _OSWIN32_
#define GET_INT1(x)     (GET_INT(x) & 0x000000ff)
#define GET_INT2(x)     (GET_INT(x) & 0x0000ffff)
#define GET_INT3(x)     (GET_INT(x) & 0x00ffffff)
#else
#define GET_INT1(x)     (GET_INT(x) & 0xff000000)
#define GET_INT2(x)     (GET_INT(x) & 0xffff0000)
#define GET_INT3(x)     (GET_INT(x) & 0xffffff00)
#endif
...
BOOL gos_str_same(CHAR *szSrc, CHAR *szDst)
{
    for (;; szSrc++, szDst++)
    {
        if (*szSrc != *szDst)
...
{
    UINT8   *pucKey = (UINT8*)pKey;

    return (pucKey[0] << 16) + (pucKey[1] << 8) + pucKey[2];
}

UINT32 gos_hash_byte5(VOID* pKey)
{
    return GET_INT(pKey) + *(((UINT8*)pKey)+4);
}

UINT32 gos_hash_byte6(VOID* pKey)
{
    return GET_INT(pKey) + GET_SHORT(((UINT32*)pKey)+1);
}

UINT32 gos_hash_byte7(VOID* pKey)
{
    UINT8   *pucKey = ((UINT8*)pKey)+4;

    return GET_INT(pKey) + (pucKey[0] << 16) + (pucKey[1] << 8) + pucKey[2];
}

UINT32 gos_hash_short(VOID *pKey)
{
    return GET_SHORT(pKey);
}
...
{
    UINT32  *pulKey = (UINT32*)pKey;

    return pulKey[0] + pulKey[1];
}

UINT32 gos_hash_ptr(VOID *pKey)
{
#ifdef GSP_32BYTE_OS
    return GET_INT(pKey) >> 2;
#else
    return ((UINT32)GET_INT64(pKey)) >> 3;
#endif
}

BOOL gos_hash_key_cmp_ptr(VOID* pSrcKey, VOID *pDestKey)
{
    if (NULL == pSrcKey || NULL == pDestKey)
    {
        return FALSE;
    }

#ifdef GSP_32BYTE_OS
    return GET_INT(pSrcKey) == GET_INT(pDestKey);
#else
    return GET_INT64(pSrcKey) == GET_INT64(pDestKey);
#endif
}
...
UINT32 gos_hash_string(VOID *pKey)
{
    ...
    for (i=0; *szKey && i<16; i+=4)
    {
        if (szKey[1] == 0)
        {
            //ulValue += szKey[0];
            ulValue += GET_INT1(szKey);
            break;
        }

        if (szKey[2] == 0)
        {
            //ulValue += ((szKey[0] << 8) + szKey[1]);
            ulValue += GET_INT2(szKey);
            break;
        }

        ulValue += *((UINT32*)szKey);
        if (szKey[3] == 0)
        {
            break;
        }
    ...
    CHAR    *szKey = (CHAR*)pKey;
    //    UINT32  ulSeed = 131;  //31, 131, 1313, 13131, 131313 etc..
    UINT32  ulValue = 0;

    while(*szKey)
    {
        // 131 = 128(2**7) + 2 + 1;
        ulValue = (ulValue<<7)+(ulValue<<1)+ulValue + (*szKey++);
    }

    return ulValue;//(hash & 0X7FFFFFFF);
...
{
    CHAR    *szKey = (CHAR*)pKey;
    UINT32  ulValue = 1315423911;

    while(*szKey)
    {
        ulValue ^= ((ulValue << 5) + (*szKey++) + (ulValue >> 2));
    }
...
{
    return strcmp((CHAR*)pSrcKey, (CHAR*)pDestKey) == 0;
}

BOOL gos_hash_key_cmp_string_ex(VOID* pSrcKey, VOID *pDestKey, UINT32 ulSrcKeyLen)
{
    UINT32      *pulSrcKey;
    UINT32      *pulDestKey;
    //UINT32      ulSrcKeyLen = strlen((CHAR*)pSrcKey);

    switch (ulSrcKeyLen)
    {
        case 0:
            return 0 == GET_INT1(pDestKey);

        case 1:
        case 2:
            return GET_INT2(pSrcKey) == GET_INT2(pDestKey);

        case 3:
            return GET_INT(pSrcKey) == GET_INT(pDestKey);

        case 4:
            pulDestKey = (UINT32*)pDestKey;

            return GET_INT(pSrcKey) == *pulDestKey++;

        case 5:
        case 6:
            pulSrcKey  = (UINT32*)pSrcKey;
            pulDestKey = (UINT32*)pDestKey;
            return *pulSrcKey++ == *pulDestKey++ &&
                   GET_INT2(pulSrcKey) == GET_INT2(pulDestKey);
        ...
        case 8:
            ...
            return *pulSrcKey++ == *pulDestKey++ &&
                   *pulSrcKey == *pulDestKey ;

    }

    pulSrcKey  = (UINT32*)pSrcKey;
    pulDestKey = (UINT32*)pDestKey;
    while(ulSrcKeyLen >= sizeof(UINT32))
    {
        if (*pulSrcKey++ != *pulDestKey++)
        {
            return FALSE;
        }

        ulSrcKeyLen -= sizeof(UINT32);
    }
    ...
    switch (ulSrcKeyLen)
    {
        case 1:
        case 2:
            return GET_INT2(pulSrcKey) == GET_INT2(pulDestKey);
...
* 说    明:
******************************************************************************/
GOS_HASH_NODE_T *gos_hash_alloc_node(GOS_HASH_QUE_T* pstHashQue, VOID *pData, UINT32 ulDataLen)//GOS_HASH_NODE_T *pstHashEntry)
{
...
* 说    明:
******************************************************************************/
VOID gos_hash_free_node(GOS_HASH_QUE_T *pstHashQue, GOS_HASH_NODE_T *pstHashNode)
{
...
* 参数名称          输入/输出               描述
  ...
  * 说    明:
******************************************************************************/
VOID* _gos_hash_add(GOS_HASH_QUE_T* pstHashQue, UINT32 ulHashValue, VOID* pData, UINT32 ulDataLen)
{
    ...
    while(pstHashNode && pstHashNode != (GOS_HASH_NODE_T *)&pstHashEntry->stNodeDummy && pstHashNode->pData)
    {
        ...
        if (pstHashQue->pfKeyCompFunc(pSrcKey, pDstKey))
        {
            ...
            /* 释放该节点 */
            gos_hash_remove_node((GHASH*)pstHashQue, (GHASHNODE*)pstHashNode);
            break;
        }
    ...
    /* 获取一个空闲的节点 */
    pstHashNode = gos_hash_alloc_node(pstHashQue, pData, ulDataLen);
    if (NULL == pstHashNode)
    {
        return NULL;
    }
...
* 说    明:
******************************************************************************/
VOID* gos_hash_add(GHASH* hHashQue, VOID* pData, UINT32 ulDataLen)
{
    GOS_HASH_QUE_T      *pstHashQue = (GOS_HASH_QUE_T*)hHashQue;
    UINT32              ulHashValue;
    ...
    return _gos_hash_add(pstHashQue, ulHashValue, pData, ulDataLen);
}
...
* 说    明:
******************************************************************************/
GOS_HASH_NODE_T* gos_hash_get_node(GHASH* hHashQue, VOID* pKey)
{
    UINT32              ulHashValue;
    GOS_HASH_ENTRY_T    *pstHashEntry;
    GOS_HASH_NODE_T     *pstHashNode;
...
VOID *gos_hash_get(GHASH* hHashQue, VOID* pKey)
{
    ...
    if (pstHashQue->pfHashFunc == gos_hash_int)
    {
        return gos_hash_get_int(hHashQue, GET_INT(pKey));
    }

    pstHashNode = (GOS_HASH_NODE_T*)gos_hash_get_node(hHashQue, pKey);
    if (NULL == pstHashNode)
    {
        return NULL;
    }
...
* 说    明:
******************************************************************************/
VOID _gos_hash_remove_node(GOS_HASH_QUE_T* pstHashQue, GOS_HASH_ENTRY_T *pstHashEntry, GOS_HASH_NODE_T* pstHashNode)
{
    ...
    gos_hash_free_node(pstHashQue, pstHashNode);
}
...
* 参数名称          输入/输出               描述
  ...
  * 说    明:
******************************************************************************/
VOID gos_hash_remove_node(GHASH* hHashQue, GHASHNODE* hNode)
{
    GOS_HASH_QUE_T      *pstHashQue = (GOS_HASH_QUE_T*)hHashQue;
    ...
    _gos_hash_remove_node(pstHashQue, pstHashEntry, (GOS_HASH_NODE_T*)hNode);
}

/******************************************************************************
* 函数名称: gos_hash_remove
* 功    能: 在HASH队列中删除一个节点
* 参    数:
* 参数名称          输入/输出               描述
...
* 说    明:
******************************************************************************/
BOOL _gos_hash_remove(GOS_HASH_QUE_T* pstHashQue, UINT32 ulHashValue, VOID* pKey)
{
    GOS_HASH_ENTRY_T    *pstHashEntry;
    GOS_HASH_NODE_T     *pstHashNode;
    UINT8*              pSrcKey;
    ...
    while(pstHashNode && pstHashNode != (GOS_HASH_NODE_T *)&pstHashEntry->stNodeDummy && pstHashNode->pData)
    {
        ...
        if (pstHashQue->pfKeyCompFunc(pSrcKey, pKey))
        {
            _gos_hash_remove_node(pstHashQue, pstHashEntry, pstHashNode);
            return TRUE;
        }
...
BOOL gos_hash_remove(GHASH* hHashQue, VOID* pKey)
{
    ...
    return _gos_hash_remove(pstHashQue, ulHashValue, pKey);
}
...
BOOL gos_hash_remove_string(GHASH* hHashQue, CHAR *szKey)
{
    ...
    return gos_hash_remove(hHashQue, szKey);
}
...
BOOL gos_hash_remove_int(GHASH* hHashQue, UINT32 ulKey)
{
    ...
    return _gos_hash_remove(pstHashQue, ulHashValue, &ulKey);
}
...
BOOL gos_hash_remove_int64(GHASH* hHashQue, UINT64 u64Key)
{
    ...
    return _gos_hash_remove(pstHashQue, ulHashValue, &u64Key);
}

/******************************************************************************
* 函数名称: gos_hash_remove_all
* 功    能: 释放所有Hash队列节点
* 参    数:
* 参数名称          输入/输出               描述
* 函数返回:
...
BOOL gos_hash_remove_all(GHASH* hHashQue)
{
    ...
    for (i=0; i<pstHashQue->ulHashEntrySize; i++)
    {
        ...
        while(pstNode && pstNode != (GOS_HASH_NODE_T *)&pstHashEntry->stNodeDummy)
        {
            ...
            _gos_hash_remove_node(pstHashQue, pstHashEntry, pstNode);
            pstNode = pstNextNode;
        }
    }
...
BOOL gos_hash_foreach(GHASH* hHashQue, GOS_HASH_FOREACH_FUNC pfFunc, VOID *pData)
{
    ...
    for (i=0; i<pstHashQue->ulHashEntrySize; i++)
    {
        ...
        while(pstNode && pstNode != (GOS_HASH_NODE_T *)&pstHashEntry->stNodeDummy && pstNode->pData)
        {
            pstNextNode = pstNode->pstNext;
...
GHASHNODE* gos_hash_get_first(GHASH* hHashQue)
{
    ...
    for (i=0; i<pstHashQue->ulHashEntrySize; i++)
    {
        ...
        pstNode = pstHashEntry->stNodeDummy.pstHead;
        if (pstNode && pstNode != (GOS_HASH_NODE_T *)&pstHashEntry->stNodeDummy && pstNode->pData)
        {
            return (GHASHNODE*)pstNode;
        }
    }
...
* 说    明:
******************************************************************************/
GHASHNODE* gos_hash_get_next(GHASH* hHashQue, GHASHNODE* hNode)
{
    GOS_HASH_QUE_T      *pstHashQue = (GOS_HASH_QUE_T*)hHashQue;
    GOS_HASH_ENTRY_T    *pstHashEntry;
    ...
    ulHashValue = pstNode->ulHashValue;
    ...
    for (i=ulHashValue+1; i<pstHashQue->ulHashEntrySize; i++)
    {
        pstHashEntry = &pstHashQue->pstHashEntry[i];
...
* 说    明:
******************************************************************************/
VOID* gos_hash_get_node_data(GHASHNODE* hNode)
{
    GOS_HASH_NODE_T     *pstNode = (GOS_HASH_NODE_T*)hNode;
...
UINT32 gos_hash_get_node_data_len(GHASHNODE* hNode)
{
    GOS_HASH_NODE_T     *pstNode = (GOS_HASH_NODE_T*)hNode;
...
GHASHNODE* gos_hash_next(GHASH* hHash, GHASHNODE* hNode, GHASHNODE **pNext)
{
    GHASHNODE *hNext = gos_hash_get_next(hHash, hNode);
