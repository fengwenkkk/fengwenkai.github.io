...
extern BOOL IsATSValid();
extern BOOL IsNTPValid();

#ifdef __BORLANDC__
extern BOOL IsMDCValid();
#endif

BOOL IsLocalAddr(const CHAR *szThreadName, UINT8 *pucAddr)
{
    ...
    sprintf(acFile, "%s/ip_%s_%s.info", gos_get_root_path(), acProcName, szThreadName);
...
BOOL ThreadOma::SendDiscoveryTrap()
{
    ...
    INT32                   iRet = -1;
    CHAR                    acModelName[32] = {OMC_MODEL_NAME_DIS};
    UINT32                  ulDevType = OMC_DEV_TYPE_DIS;
...
BOOL ThreadOma::InitOmaAddr()
{
    ...
    if (IsLocalAddr("OMA", m_aucOmcAddr))
    {
                              ...
                              unsigned int ulMaxOIDNum, unsigned int *pulOIDNum)
{
    ...
    *pulGetType = *pucRspData++;
    ...
    if (pucRspData[0] != ASN_INTEGER ||
        pucRspData[1] == 0)
    {
        OmaLog(LOG_ERROR, "snmp_parse_get_req: check reqid faile, pucRspData[0]:%u,pucRspData[1]:%u.",
...
#ifndef __BORLANDC__
    if (IsLocalAddr("OMA", m_aucMasterAddr))
    {
...
BOOL ThreadOma::SendGetSysStatusRsp(VOID *pvSession, UINT32 ulReqID)
{
    ...
    SNMP_VAR_T      *pstVarList = NULL;
    INT32           iRet = -1;
    BOOL            bATSValid = IsATSValid();
    BOOL            bNTPValid = IsNTPValid();
    BOOL            bMDCValid;

#ifndef __BORLANDC__
    bMDCValid = gos_simple_ping(m_aucMdcAddr, 300);
#else
    bMDCValid = IsMDCValid();
#endif

    SNMP_ADD_SCALAR_VAR(pstVarList, oid_ATSStatus,      TYPE_INT,   bATSValid);
    SNMP_ADD_SCALAR_VAR(pstVarList, oid_NTPStatus,      TYPE_INT,   bNTPValid);
    SNMP_ADD_SCALAR_VAR(pstVarList, oid_LTEStatus,      TYPE_INT,   bMDCValid);
...
BOOL ThreadOma::CheckAlarm()
{
    ...
    stAlarmData.bATSValid = IsATSValid();
    ...
    stAlarmData.bLTEValid = gos_simple_ping(m_aucMdcAddr, 300);
#else
    stAlarmData.bLTEValid = IsMDCValid();
    stAlarmData.bIsMasterStatus = FALSE;
#endif
    ...
    for (i=0; i<m_ulOmaInfoNum; i++)
    {
        ...
        aiAlarmStatus[ATS_ALARM] += m_astAlarmData[i].bATSValid?-1:1;
        aiAlarmStatus[LTE_ALARM] += m_astAlarmData[i].bLTEValid?-1:1;
    }
